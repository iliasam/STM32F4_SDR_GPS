...
{
	uint8_t j = 4;
	if (pntpos(obs_p, j, &nav_data, &gps_sol))
	{
...
static double varerr(double el, int sys)
{
	double fact, varr;
	fact = EFACT_GPS;
	...
	double *resp, int *ns)
{
	...
	double vion = 0;
	double vtrp = 0;
	double rr[3], pos[3], e[3], P, lam_L1;
	...
	for (i = *ns = 0; i < n && i < MAXSAT; i++) 
	{
		...
		/* error variance */
		var[nv++] = varerr(azel[1 + i * 2], sys) + vare[i] + vmeas + vion + vtrp;
